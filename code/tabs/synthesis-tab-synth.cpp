...
C0C1Chart::Sample to_sample(const numina::RegulatorDesigner::Settings& s) {
...
void SynthesisTab::sync_c0c1_selection_from_params() {
    ...
    if (kp > 0.0 && tu > 0.0 && std::isfinite(kp) && std::isfinite(tu))
        ui->c0c1Chart->setSelection(kp / tu, kp);
    else
...
void SynthesisTab::apply_design(const numina::RegulatorDesigner::Design& d, bool replace_last) {
    if (!d.isOk())
        return;
    const auto& s    = d.settings;
    const bool use_d = d.law == numina::RegulatorDesigner::Law::Pid && s.td > 0.0 && std::isfinite(s.td);
    block_param_signals(true);
...
void SynthesisTab::onSamplePicked(const C0C1Chart::Sample& sample) {
    if (!(sample.c0 > 0.0) || !(sample.c1 > 0.0) || !(sample.kp > 0.0) || !(sample.tu > 0.0))
        return;
    apply_pi_settings(sample.kp, sample.tu, !ui->charts->empty());
}
...
void SynthesisTab::autoSynthesize() {
    ...
    try {
        ...
        const auto bundle = regulator_design::synthesize(designer, spec, selected_criterion());
        const auto loc    = regulator_design::locus(designer, spec.law);
